...
function apparently(string) {
    ...
    //create array to push modified strings too
    let finalArr = [];
    //create loop that finds and/but
    for(let i = 0; i < arr.length; i++){
        //if and/but is followed by apparently, just return the string (no duplicates)
      if(arr[i] == 'and' && arr[i + 1] == 'apparently' || arr[i] == 'but' && arr[i + 1] == 'apparently'){
        finalArr.push(arr[i])
      }
      //in case of and/but without apparently already, add apparently
      else if(arr[i] == 'and' || arr[i] == 'but'){
        finalArr.push([arr[i] + ' '  + 'apparently'])
      }
      //return string with no modifications
      else{
        finalArr.push(arr[i])
      }
    }
